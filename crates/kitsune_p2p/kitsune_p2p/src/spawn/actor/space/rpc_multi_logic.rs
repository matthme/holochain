...
pub(crate) async fn handle_rpc_multi(
    ...
    tracing::warn!(elapsed = %start.elapsed().as_secs_f64(), "RPC_MULTI");
...
pub(crate) async fn handle_rpc_multi_as_single(
    ...
    let make_req = move |
        con_hnd: Tx2ConHnd<crate::wire::Wire>,
    ...
    max_timeout.mix("rpc_multi", async move {
        ...
        for agent in local_joined_agents {
            match ro_inner.evt_sender.call(space.clone(), agent.clone(), payload.clone()).await {
                Ok(response) => {
                    return Ok(vec![RpcMultiResponse {
                        agent,
                        response,
                    }]);
                }
